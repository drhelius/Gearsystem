...
This skill requires the **Gearsystem MCP server** to be installed and connected. All ROM hacking operations are performed through Gearsystem MCP tools. Verify connectivity by calling `debug_get_status` — if it returns a response, the server is active.
...
Gearsystem runs as an MCP server using STDIO transport (recommended). Configure your AI client:
