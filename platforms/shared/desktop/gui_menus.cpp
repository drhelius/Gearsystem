...
static void menu_emulator(void)
{
    if (ImGui::BeginMenu("Emulator"))
    {
        ...
        if (ImGui::BeginMenu("System"))
        {
            ...
            ImGui::Combo("##emu_system", &config_emulator.system, "Auto\0Master System / Mark III\0Game Gear (2 ASIC)\0Game Gear (2 ASIC) SMS Mode\0Game Gear (1 ASIC)\0Game Gear (1 ASIC) SMS Mode\0SG-1000 / Multivision\0SG-1000 II\0\0");
            ImGui::PopItemWidth();
            ImGui::EndMenu();
        }
...
static void menu_input(void)
{
    if (ImGui::BeginMenu("Input"))
    {
        ...
        bool allow_up_down = config_input[0].allow_up_down && config_input[1].allow_up_down;

        if (ImGui::MenuItem("Allow Up+Down / Left+Right", "", &allow_up_down))
        {
            for (int i = 0; i < GS_MAX_GAMEPADS; i++)
                config_input[i].allow_up_down = allow_up_down;
        }
        if (ImGui::IsItemHovered())
        {
            ImGui::BeginTooltip();
