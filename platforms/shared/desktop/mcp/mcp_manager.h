...
private:
    ...
    json start_input_macro(const json& arguments, const json& request_id)
    {
        ...
        if (default_player < 1 || default_player > 2)
        {
    ...
    void pump_input_macro(GearsystemCore* core)
    {
    ...
    void continue_input_macro_wait(GearsystemCore* core)
    {
        if (emu_is_debug_idle())
        {
            if (core->GetProcessor()->BreakpointHit())
            {
    ...
    void handle_controller_side_effects(GearsystemCore* core, json& result)
    {
