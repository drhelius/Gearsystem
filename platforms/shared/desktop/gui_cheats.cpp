...
static bool cheat_code_normalize(const char* input, char* output, size_t output_size)
{
    ...
    if ((length == 7) && (code[4] == ':') && cheat_code_is_hex_format(code, 7, 4, -1))
    {
        snprintf(output, output_size, "00%c%c%c%c%c%c", code[0], code[1], code[2],
    ...
    if ((length == 9) && ((code[4] == ' ') || (code[4] == '-') || (code[4] == ':')) &&
        cheat_code_is_hex_format(code, 9, 4, -1))
    {
        snprintf(output, output_size, "%c%c%c%c%c%c%c%c", code[0], code[1], code[2],
            code[3], code[5], code[6], code[7], code[8]);
        return true;
    }
...
void gui_cheats_window(void)
{
    ...
    ImGui::TextDisabled("GG: XXX-XXX[-XXX]      PAR: 00AAAADD or AAAA:DD");
    ...
    if (ImGui::BeginTable("cheats_table", 4, table_flags, ImVec2(0.0f, table_height)))
    {
        ...
        for (std::list<gui_CheatEntry>::iterator it = cheat_list.begin(); it != cheat_list.end(); it++, row++)
        {
            ...
            {
                ImGui::SetTooltip("Use XXX-XXX, XXX-XXX-XXX, 00AAAADD, or AAAA:DD.");
            }
